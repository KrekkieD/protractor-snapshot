...
module.exports = {
    extract: extract,
    defaultConfig: _getDefaultConfig()
};
...
function extract (providedConfig) {
    ...
    config.source.removeMetaFragments = typeof config.source.removeMetaFragments === 'boolean' ? config.source.removeMetaFragments : defaultConfig.source.removeMetaFragments;
    ...
    config.report = typeof providedConfig.report !== 'undefined' ? providedConfig.report : defaultConfig.report;
    return config;
...
function _getDefaultConfig () {
    ...
    return {
        ...
        defaultResolution: [1024, 768, 'window'],
        resolutions: [
            [1366, 768, 'window'],
            [768, 1024, 'window'],
            [1920, 1080, 'window'],
...
function _validateResolution (resolution) {
    ...
    if (typeof resolution[1] !== 'number' && resolution[1].toLowerCase() !== 'full') {
        throw _errorHandler('resolution[1] (height) must be a number or \'full\', got ' + typeof resolution[1], { resolution: resolution });
    }
